...
export class CalendarComponent {
  ...
  addAppointment(
    ...
    this._CalenderService.appointments.push({
  ...
  getAppointmentsForDate(day: Date, timeSlots: string[]) {
    return this._CalenderService.appointments
      .filter((appointment) => {
  ...
  getAppointmentsForDateTime(date: Date, timeSlot: string): Appointment[] {
    const appointmentsForDateTime: Appointment[] = this._CalenderService.appointments.filter(
      (appointment) =>
  ...
  editAppointment(appointment: Appointment, event: Event) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const index = this._CalenderService.appointments.findIndex(
          (appointment) => appointment.uuid === result.uuid
        );
        if (result.remove) {
          this._CalenderService.appointments.splice(index, 1);
        } else {
          this._CalenderService.appointments[index] = result;
        }
      }
    });
  }
